  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <main className="max-w-md mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/60 border border-slate-100 overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                <div className="relative">
                  ...
                  <input type="text" placeholder="Tu nombre" className="w-full pl-12 pr-4 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" />
                </div>
                <div className="relative">
                  <Phone className="absolute left-4 top-4 text-slate-300 w-5 h-5" />
                  <input type="tel" placeholder="Tu teléfono" className="w-full pl-12 pr-4 py-4 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-medium" />
                </div>
                <button 
                  onClick={() => setStep(4)}
                  className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black text-lg hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all active:scale-95"
                >
                  Confirmar Cita
                </button>
              </div>
          ...
          {step === 4 && (
            <div className="p-10 text-center space-y-6">
              ...
              <div>
                <h3 className="text-2xl font-bold text-slate-800 tracking-tight">¡Todo listo!</h3>
                <p className="text-slate-500 mt-2 leading-relaxed">
                  Tu reserva para <strong>{selectedService?.name}</strong> a las <strong>{selectedTime}</strong> ha sido procesada.
                </p>
              </div>
              <div className="bg-slate-50 p-4 rounded-2xl border border-dashed border-slate-200">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Negocio:</p>
                <p className="font-bold text-slate-700">TicoFlow - Barbería & Estilo</p>
              </div>
              <button 
                onClick={() => setStep(1)}
        ...
        <p className="text-center mt-12 text-slate-400 text-xs font-bold tracking-widest uppercase">
          TicoFlow MVP · 2026
        </p>
      </main>
    </div>
